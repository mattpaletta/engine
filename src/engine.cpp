...
void Engine::init_opengl() {
    ...
    this->window = glfwCreateWindow(this->SCREEN_SIZE.WIDTH, this->SCREEN_SIZE.HEIGHT, this->game->name.c_str(), nullptr, nullptr);
...
float Engine::getScaleRatio() const {
    ScreenSize scaled_size;
    glfwGetFramebufferSize(this->window, &scaled_size.WIDTH, &scaled_size.HEIGHT);
    return (float) scaled_size.WIDTH / (float) this->SCREEN_SIZE.WIDTH;
}

int Engine::getScaledWidth() const {
    return (int) (this->SCREEN_SIZE.WIDTH * this->getScaleRatio());
}

int Engine::getScaledHeight() const {
    return (int) (this->SCREEN_SIZE.HEIGHT * this->getScaleRatio());
}

float Engine::scaleConst(const float& desired_size) const {
    return desired_size * this->getScaleRatio();
}

Size Engine::scaleObj(const Size& desired_size) const {
    const float SCALE_CONSTANT = this->getScaleRatio();
...
#if ENGINE_ENABLE_VR
        return !glfwWindowShouldClose(this->window) || this->vr.ShouldShutdown();
#else
...
void Engine::Update_VR_vibration(const int leftStrength, const int rightStrength) {
#if ENGINE_ENABLE_VR
